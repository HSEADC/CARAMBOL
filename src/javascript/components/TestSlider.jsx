...
const TestSlider = ({ tests }) => {
  ...
  const totalSlides = Math.ceil(tests.length / itemsPerSlide)
  ...
  return (
    <>
        ...
        <div className="W_sliderContainer">
          ...
          >
            {tests.map((test, index) => (
              <TestCard key={index} {...test} onClick={() => openQuiz(test.testData)} />
            ))}
          </div>
        </div>
      ...
      <QuizModal isOpen={isQuizOpen} onClose={closeQuiz} testData={selectedTest} />
    </>
  )
}
